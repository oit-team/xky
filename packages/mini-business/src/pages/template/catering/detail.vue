...
<template>
  <container classes="flex flex-col bg-gray-100">
    ...
    <div class="flex flex-col gap-3 p-3">
      ...
      <van-cell-group>
        <van-cell :title="getLabel('meatAndVegetable')" :value="getValue('productNo')" />
        <van-cell :title="getLabel('productFlavor')" :value="getValue('productFabric')" />
        <van-cell :title="getLabel('productTypeName')" :value="getValue('productTypeName')" />
        <van-cell :title="getLabel('productMaterials')">
          <template>
            <rich-text :nodes="getValue('productMaterials')" />
          </template>
        </van-cell>
      </van-cell-group>
